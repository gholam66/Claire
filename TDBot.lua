...
function changeDes(Claire,TDBot)
assert (tdbot_function ({
_ = 'changeChannelDescription',
channel_id = getChatId(Claire).id,
description = TDBot
}, dl_cb, nil))
end
...
function getChannelFull(Claire,TDBot)
assert (tdbot_function ({
_ = 'getChannelFull',
channel_id = getChatId(Claire).id
}, TDBot, nil))
...
local function Joinch(TDBot,Claire)
if Claire.first_name then
username = "( "..Claire.first_name.." )"
SendMetion(msg.chat_id,msg.sender_user_id, msg.id,'⌯ کاربر :\n'..username..'\nشما باید در کانال ربات عضو شوید و سپس دستور ارسال کنید\nکانال ما :\n[ '..Channel..' ]',10,utf8.len(username))
end
end
...
if Stats == "Warn" then
local function Warnmsg(TDBot,Claire)
if Claire.first_name then
username = "( "..Claire.first_name.." )"
end
...
if Stats == "Kick" then
local function Kickmsg(TDBot,Claire)
if Claire.first_name then
username = "( "..Claire.first_name.." )"
end
SendMetion(msg.chat_id,msg.sender_user_id, msg.id,"⌯ کاربر :\n"..username.."\n اخراج شد\nعلت اخراج : "..type.."",10,utf8.len(username))
KickUser(msg.chat_id,msg.sender_user_id)
RemoveFromBanList(msg.chat_id,msg.sender_user_id)
end
...
if not redis:sismember('TDBotAll',msg.chat_id) then
redis:sadd('TDBotAll',msg.chat_id)
...
sendText(msg.chat_id,msg.id,"⌯ کاربر :\n ( "..msg.sender_user_id.." )\nشما باید ("..forcemax..") نفر به گروه اضافه کنید تا بتوانید پیام ارسال کنید","md")
...
SendMetion(msg.chat_id,msg.sender_user_id, msg.id, '⌯ کاربر :\n( '..msg.sender_user_id..' )\nبه علت ارسال بیش از حد پیام  از گروه اخراج شد', 10,string.len(msg.sender_user_id))
KickUser(msg.chat_id,msg.sender_user_id)
end
...
SendMetion(msg.chat_id,msg.sender_user_id, msg.id, '⌯ کاربر :\n( '..msg.sender_user_id..' )\nبه علت ارسال بیش از حد پیام پاک شد', 10,string.len(msg.sender_user_id))
...
SendMetion(msg.chat_id,msg.sender_user_id, msg.id, '⌯ کاربر :\n( '..msg.sender_user_id..' )\nبه علت ارسال بیش از حد پیام در گروه محدود شد', 10,string.len(msg.sender_user_id))
...
MsgType = 'text'
 ...
 if msg.content._== "messageText" then
local function GetM(TDBot,Claire) 
print("\027[" ..color.blue[1].. "m["..os.date("%H:%M:%S").."]\027[00m ["..Claire.title.."] "..msg.sender_user_id.." >>>> "..msg.content.text.."")
end
GetChat(msg.chat_id,GetM)
end
if msg.content.caption then
function GetM(TDBot,Claire)
function GetName(TDBots,TDBot)
print("["..os.date("%H:%M:%S").."] "..Claire.title.." "..msg.sender_user_id.." >>>> "..msg.content.caption.."")
end
...
function GetM(TDBot,Claire)
function GetName(TDBots,TDBot)
print("["..os.date("%H:%M:%S").."] "..Claire.title.." >>>> Joined By link "..TDBot.first_name.."")
end
GetUser(msg.sender_user_id,GetName)
end
GetChat(msg.chat_id,GetM)
...
function GetM(TDBot,Claire)
function GetName(TDBots,TDBot)
print("["..os.date("%H:%M:%S").."] "..Claire.title.." "..TDBot.first_name.." >>>>[messageDocument][  "..TDBot.id.."]")
MsgType = 'Document'
end
GetUser(msg.sender_user_id,GetName)
end
...
SendMetion(msg.chat_id,msg.sender_user_id, msg.id, '⌯ کاربر :\n( '..msg.sender_user_id..' )\nشما در لیست بن همگانی ربات قرار دارید', 12,string.len(msg.sender_user_id))
KickUser(msg.chat_id,msg.sender_user_id)
end

if MsgType == 'AddUser' then
function ByAddUser(Claire,TDBot)
if is_GlobalyBan(TDBot.id) then
print '                      >>>>Is  Globall Banned <<<<<       '
sendText(msg.chat_id, msg.id,'⌯ کاربر :\n( '..TDBot.id..' )\nدر لیست بن همگانی قرار دارد','md')
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nدر لیست بن همگانی قرار دارد', 12,string.len(TDBot.id))
end
...
KickUser(msg.chat_id,msg.sender_user_id)
SendMetion(msg.chat_id,msg.sender_user_id, msg.id, '⌯ کاربر : ( '..msg.sender_user_id..' )\nشما از این گروه محروم شده اید', 12,string.len(msg.sender_user_id))
else
function WelcomeByLink(Claire,TDBot)
...
local txtt = txtt:gsub('{first}',TDBot.first_name)
local txtt = txtt:gsub('{rules}',ruless)
local txtt = txtt:gsub('{link}',linkk)
local txtt = txtt:gsub('{last}',TDBot.last_name or '')
local txtt = txtt:gsub('{username}','@'..TDBot.username or '')
...
SendMetion(msg.chat_id,msg.add, msg.id, '⌯ کاربر :( '..msg.add..' )\nشما از این گروه محروم شده اید', 12,string.len(msg.add))
else
function WelcomeByAddUser(Claire,TDBot)
print('New User : \nChatID : '..msg.chat_id..'\nUser ID : '..msg.add..'')
if redis:get('Text:Welcome:'..msg.chat_id) then
txtt = redis:get('Text:Welcome:'..msg.chat_id)
else
...
if msg.add then
function forceAdd(Claire,TDBot)
if TDBot.type._ == "userTypeBot" then
print '               Bot added              '  
sendText(msg.chat_id, msg.id, '⌯ کاربر :\n ( '..ec_name(TDBot.first_name)..' )\nشما نمیتوانید ربات را به گروه اضافه کنید ', 'md')
KickUser(msg.chat_id,TDBot.id)
else
...
sendText(msg.chat_id,msg.id,"⌯ کاربر :\n( "..ec_name(TDBot.first_name).." )\nشما یک نفر را به گروه اضافه کردید \nتعداد مانده : "..forcemax.."/"..added,"md")
...
if msg.content._ == "messageChatJoinByLink" or msg.content._ == "messageChatAddMembers" or
msg.content._ == "messageChatAddSudo" or
msg.content._ == "messageChatAddOwner" or
msg.content._ == "messageChatAddMods" or
msg.content._ == "messageChatDeleteMember" then
deleteMessages(msg.chat_id, {[0] = msg.id})
end
end
------------------------------------------------
...
sendText(msg.chat_id, msg.id,'⌯ قفل خودکار گروه غیرفعال شد\nکاربران میتوانید پیام ارسال کنید', 'md')
...
if redis:get("MuteAll:"..msg.chat_id) then
sendText(msg.chat_id, msg.id,'⌯ قفل خودکار غیرفعال شد\n_کاربران میتوانید پیام ارسال کنید_' , 'md')
redis:del("MuteAll:"..msg.chat_id)
end
end
end
...
LockProExtra(msg,Stats,'ارسال موقعیت مکانی')
end
end
-------------------------------
...
SendMetion(msg.chat_id,sudo, msg.id, '⌯ کاربر :\n( '..sudo..' )\nبه لیست سودو های ربات اضافه شد', 10,string.len(sudo))
end
...
function SetSudouserr(Claire,TDBot)
if TDBot.id then
redis:sadd('SUDO-ID',TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nبه لیست سودو های ربات اضافه شد', 12,string.len(TDBot.id))
else 
text = '⌯ کاربر یافت نشد'
...
local function SetSudo(Claire, TDBot)
local user = TDBot.sender_user_id
redis:sadd('SUDO-ID',user)
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user.. ' )\nبه لیست سودو های ربات اضافه شد', 12,string.len(user))
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
SendMetion(msg.chat_id,sudo, msg.id, '⌯ کاربر :\n( '..sudo..' )\nاز لیست سودو ربات حذف شد', 12,string.len(sudo))
end
...
function SetSudouserr(Claire,TDBot)
if TDBot.id then
redis:srem('SUDO-ID',TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nاز لیست سودو ربات حذف شد', 12,string.len(TDBot.id))
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
end
...
local function RemSudo(Claire, TDBot)
local user = TDBot.sender_user_id
redis:srem('SUDO-ID',user)
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nاز لیست سودو ربات حذف شد', 12,string.len(user))
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
if matches == 'sudolist' or matches == 'لیست سودو' then
local hash =  "SUDO-ID"
...
4 -` دارای قابلیات پاکسازی (پیام های گروه-لیست محدود-لیست بن-و...)`
...
جهت خرید به ]]..check_markdown(UserSudo)..[[ یا ]]..check_markdown(PvUserSudo)..[[ مراجعه کنید
]]
sendText(msg.chat_id, msg.id, text,'md')
...
function Test(Claire,TDBot)
local text = TDBot.content.text
local list = redis:smembers('TDBotAll')
for k,v in pairs(list) do
...
if matches == 'reload' or matches == 'ریلود' then
sendText(msg.chat_id,msg.id,'⌯ سیستم ربات '..botname..' بروزرسانی شد','md')
dofile('./Bot/TDBot.lua')
...
local text = '⌯ گروه :\n( '..TDBot.title..' )\nدر لیست گروه های مدیریتی قرار ندارد'
 sendText(msg.chat_id, msg.id,text,'md')
else
local text = '⌯ گروه :\n( '..TDBot.title..' )\nاز لیست گروه های مدیریتی حذف شد'
local Hash = 'StatsGpByName'..msg.chat_id
redis:del(Hash)
 sendText(msg.chat_id, msg.id,text,'md')
...
sendText(msg.chat_id,msg.id,'⌯ ربات از گروه\n( '..chat_id..' )\nبا موفقیت خارج شد','md')
sendText(chat_id,0,'','md')
Left(chat_id,TD_ID, "Left")
end 
if matches == 'gplist' or matches == 'لیست گروه ها' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
get(BotHelper, msg.chat_id, 0, 0, "chats",0, GetPanel)
end
if matches == 'gplist pv' or matches == 'گروه ها خصوصی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ لیست گروه های ربات به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
sendDocument(Sudoid, 0, 0, 1, nil, './Ci.lua', dl_cb, nil)
sendDocument(Sudoid, 0, 0, 1, nil, './Config.lua', dl_cb, nil)
sendDocument(Sudoid, 0, 0, 1, nil, './Ai.lua', dl_cb, nil)
...
local text = '⌯ گروه ( '..TDBot.title..' ) شارژ شد  به مدت  [ '..ti..' ] روز'
sendText(msg.chat_id, msg.id,text,'md')
if redis:get('CheckExpire:'..msg.chat_id) then
 redis:set('CheckExpire:'..msg.chat_id,true)
end
...
local text =[[
تمام پیام های ارسالی : ]]..allmsgs..[[

لیست سوپر گروه ها :]]..supergroup..[[
...
┅┅┅┅┅┅┅┅┅┅┅┅┅
 ...
 آپتایم ]]..botname..[[ : ]]..uptime..[[

 مقدار استفاده شده ردیس : ]]..totalredis..[[
...
sendText(msg.chat_id, msg.id,'⌯ آمار ربات '..botname..' ریست شد و از صفر شروع شد',  'md' )
end
if matches == 'ownerlist' or matches == 'لیست مالک' then
...
local function SetRank_Rep(Claire, TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
redis:set('rank'..TDBot.sender_user_id,rank)
local user = TDBot.sender_user_id
SendMetion(msg.chat_id,user, msg.id, '⌯ مقام کاربر :\n( '..user..' )\nتغییر کرد به : [ '..rank..' ]', 17,string.len(user))
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
local function SetOwner_Rep(Claire, TDBot)
local user = TDBot.sender_user_id
if redis:sismember('OwnerList:'..msg.chat_id,user) then
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nدر لیست مالکان گروه قرار دارد', 12,string.len(TDBot.sender_user_id))
else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nبه لیست مالکان گروه اضافه شد', 12,string.len(TDBot.sender_user_id))
...
if redis:sismember('OwnerList:'..msg.chat_id,user) then
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر : '..user..' از قبل در لیست مالکان گروه قرار داشت', 12,string.len(user))
else
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر : '..user..' به لیست مالکان گروه اضافه شد', 12,string.len(user))
...
function SetOwnerByUsername(Claire,TDBot)
if TDBot.id then
print(''..TDBot.id..'')
if redis:sismember('OwnerList:'..msg.chat_id,TDBot.id) then
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nاز قبل در لیست مالکان گروه قرار داشت ', 12,string.len(TDBot.id))
else
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nبه لیست مالکان گروه اضافه شد', 12,string.len(TDBot.id))
redis:sadd('OwnerList:'..msg.chat_id,TDBot.id)
end
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
...
local function RemOwner_Rep(Claire, TDBot)
local user = TDBot.sender_user_id
if redis:sismember('OwnerList:'..msg.chat_id, TDBot.sender_user_id) then
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nاز لیست مالکان گروه حذف شد ', 12,string.len(TDBot.sender_user_id))
redis:srem('OwnerList:'..msg.chat_id,TDBot.sender_user_id)
else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nدر لیست مالکان گروه وجود ندارد', 12,string.len(TDBot.sender_user_id))
end
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
if redis:sismember('OwnerList:'..msg.chat_id,user) then
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nاز لیست مالکان گروه حذف شد ', 12,string.len(user))
redis:srem('OwnerList:'..msg.chat_id,user)
else
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nدر لیست مالکان گروه وجود ندارد',12,string.len(user))
end
end
...
function RemOwnerByUsername(Claire,TDBot)
if TDBot.id then
print(''..TDBot.id..'')
if redis:sismember('OwnerList:'..msg.chat_id, TDBot.id) then
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nاز لیست مالکان گروه پاک شد', 12,string.len(TDBot.id))
redis:srem('OwnerList:'..msg.chat_id,TDBot.id)
else
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nدر لیست مالکان گروه وجود ندارد', 12,string.len(TDBot.id))
end
else  
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
...
if matches == 'clean owners' or matches == 'پاکسازی لیست مالکان' then
redis:del('OwnerList:'..msg.chat_id)
sendText(msg.chat_id, msg.id,'⌯ لیست مالکان گروه پاکسازی شد', 'md')
end
---------Start---------------Globaly Banned-------------------
if matches == 'banall' or matches == 'بن ال' then
function GbanByReply(Claire,TDBot)
if redis:sismember('GlobalyBanned:',TDBot.sender_user_id) then
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nقبلا در لیست بن ال وجود دارد', 12,string.len(TDBot.sender_user_id))
else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nبه لیست بن ال اضافه شد', 12,string.len(TDBot.sender_user_id))
redis:sadd('GlobalyBanned:',TDBot.sender_user_id)
end
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
if matches and (matches:match('^banall (%d+)') or matches:match('^بن ال (%d+)')) then
local user = matches:match('^banall (%d+)') or matches:match('^بن ال (%d+)')
if redis:sismember('GlobalyBanned:',user) then
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nقبلا در لیست بن ال وجود دارد', 12,string.len(user))
else
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nبه لیست بن ال اضافه شد', 12,string.len(user))
redis:sadd('GlobalyBanned:',user)
end
end
if matches and (matches:match('^banall @(.*)') or matches:match('^بن ال @(.*)')) then
local username = matches:match('^banall @(.*)') or matches:match('^بن ال @(.*)')
function BanallByUsername(Claire,TDBot)
if TDBot.id then
print(''..TDBot.id..'')
if redis:sismember('GlobalyBanned:', TDBot.id) then
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nقبلا در لیست بن ال وجود دارد', 12,string.len(TDBot.id))
else
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nبه لیست بن ال اضافه شد', 12,string.len(TDBot.id))
redis:sadd('GlobalyBanned:',TDBot.id)
end
else 
text = '⇜ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
...
if matches == 'gbanlist' or matches == 'لیست بن ال' then
...
t = t.."\n\n⌯ شما میتوانید از دستور زیر برای دیدن کاربر استفاده کنید\nاطلاعات 377450049"
if #list == 0 then
t = '⌯ لیست بن ال خالی میباشد'
end
sendText(msg.chat_id, msg.id,t, 'md')
end
if matches == 'clean gbans' or matches == 'پاکسازی لیست بن ال' then
redis:del('GlobalyBanned:')
sendText(msg.chat_id, msg.id,'⌯ لیست بن بن ال شد', 'md')
end
---------------------Unbanall--------------------------------------
if matches and (matches:match('^unbanall (%d+)') or matches:match('^حذف بن ال (%d+)')) then
local user = matches:match('unbanall (%d+)') or matches:match('حذف بن ال (%d+)')
if tonumber(user) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
if redis:sismember('GlobalyBanned:',user) then
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nاز لیست بن ال حذف  شد', 12,string.len(user))
redis:srem('GlobalyBanned:',user)
else
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nدر لیست بن ال وجود ندارد', 12,string.len(user))
end
end
if matches and (matches:match('^unbanall @(.*)') or matches:match('^حذف بن ال @(.*)')) then
local username = matches:match('^unbanall @(.*)') or matches:match('^حذف بن ال @(.*)')
function UnbanallByUsername(Claire,TDBot)
if tonumber(TDBot.id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
if TDBot.id then
print(''..TDBot.id..'')
if redis:sismember('GlobalyBanned:',TDBot.id) then
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر : '..TDBot.id..' از لیست بن ال حذف  شد', 12,string.len(TDBot.id))
redis:srem('GlobalyBanned:',TDBot.id)
else
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر : '..TDBot.id..' در لیست بن ال وجود ندارد', 12,string.len(TDBot.id))
end
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
...
if matches == 'unbanall' or matches == 'حذف بن ال' then
function UnGbanByReply(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
if redis:sismember('GlobalyBanned:',TDBot.sender_user_id) then
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n '..TDBot.sender_user_id..' از لیست بن ال حذف  شد', 12,string.len(TDBot.sender_user_id))
redis:srem('GlobalyBanned:',TDBot.sender_user_id)
else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )/nدر لیست بن ال وجود ندارد', 12,string.len(TDBot.sender_user_id))
end
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
if matches == 'clean member' or matches == 'پاکسازی کاربران' then 
function CleanMembers(Claire, TDBot) 
for k, v in pairs(TDBot.members) do 
...
sendText(msg.chat_id, msg.id,'⌯ تعدادی از کاربران گروه اخراج شدند', 'md') 
end 
-------------------------------
end
if is_Owner(msg) then
...
sendText(msg.chat_id,msg.id,"⌯ تمام مدیران گروه ارتقا یافتند", "md")
end
...
if matches == 'delall' or matches == 'پاکسازی پیام ها' then
...
sendText(msg.chat_id, msg.id,'⌯ تمامی پیام های گروه درحال پاکسازی هستند' ,'md')
end
if matches == 'delall' or matches == 'پاکسازی پیام ها' then
 local function pro(arg,data)
for k,v in pairs(data.members) do
...
function PromoteByReply(Claire,TDBot)
redis:sadd('ModList:'..msg.chat_id,TDBot.sender_user_id)
local user = TDBot.sender_user_id
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nترفیع یافت', 12,string.len(user))
end
...
function DemoteByReply(Claire,TDBot)
redis:srem('ModList:'..msg.chat_id,TDBot.sender_user_id)
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nعزل مقام شد', 12,string.len(TDBot.sender_user_id))
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
function DemoteByUsername(Claire,TDBot)
if TDBot.id then
print(''..TDBot.id..'')
redis:srem('ModList:'..msg.chat_id,TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nعزل مقام شد', 12,string.len(TDBot.id))
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
end
...
function PromoteByUsername(Claire,TDBot)
if TDBot.id then
print(''..TDBot.id..'')
redis:sadd('ModList:'..msg.chat_id,TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nترفیع یافت', 12,string.len(TDBot.id))
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
end
...
----------------------
if matches1 and (matches1:match('^[Ss]etabout (.*)') or matches1:match('^تنظیم درباره (.*)')) and is_JoinChannel(msg) then
local description = matches1:match('^[Ss]etabout (.*)') or matches1:match('^تنظیم درباره (.*)')
...
SendMetion(msg.chat_id,user, msg.id, '⌯ کاربر :\n( '..user..' )\nعزل مقام شد', 12,string.len(user))
end
if (matches == 'autolock enable' or matches == 'قفل خودکار فعال') and is_JoinChannel(msg) then
redis:set('automuteall'..msg.chat_id,true)
sendText(msg.chat_id, msg.id,'⌯ قفل خودکار گروه فعال شد\nلطفا پیامی ارسال نکنید' ,'md')
end
if (matches == 'autolock disable' or matches == 'قفل خودکار غیرفعال') and is_JoinChannel(msg) then
redis:del('automuteall'..msg.chat_id)
sendText(msg.chat_id, msg.id,'⌯ قفل خودکار گروه  غیر فعال شد\nمیتوانید پیام ارسال کنید' ,'md')
end
if (matches == 'clean mods' or matches == 'پاکسازی لیست مدیران') and is_JoinChannel(msg) then
redis:del('ModList:'..msg.chat_id)
sendText(msg.chat_id, msg.id,  '⌯ لیست مدیران پاکسازی شد', 'md')
end
if matches1 and (matches1:match('^([Ss]etlockgroup) (.*)$') or matches1:match('^(قفل گروه) (.*)$')) and is_JoinChannel(msg) then
local matches1 = matches1:gsub("قفل گروه", "setlockgroup")
local status = {string.match(matches1, "^([Ss]etlockgroup) (.*)$")}
if status[2] == 'rcd' or status[2] == 'محدود' then
...
----------------
if matches and (matches1:match('^([Aa]utolock) (%d+):(%d+)-(%d+):(%d+)$') or matches1:match('^(تنظیم قفل خودکار) (%d+):(%d+)-(%d+):(%d+)$') or matches1:match('^([Ss]etautolock) (%d+):(%d+)-(%d+):(%d+)$')) and is_JoinChannel(msg) then
local matches1 = matches1:gsub("تنظیم قفل خودکار", "setautolock")
local matches = {string.match(matches1, "^([Aa]utolock) (%d+):(%d+)-(%d+):(%d+)$")}
...
if (matches == 'addkick' or matches == 'افزودن مسدودی ها')  then
local function Clean(Claire,TDBot)
for k,v in pairs(TDBot.members) do
addChatMembers(msg.chat_id,{[0] = v.user_id})
end
end
sendText(msg.chat_id, msg.id, '⌯ کاربران لیست بن همگانی به گروه اضافه شده اند', 'md')
getChannelMembers(msg.chat_id,"Banned", 0, 2000,Clean)
end
end
----
if is_Mod(msg) then
if (matches == "clean deleted" or matches == 'پاکسازی دلیت اکانت ها') and is_JoinChannel(msg) then
function list(Claire,TDBot)
for k,v in pairs(TDBot.members) do
local function Checkdeleted(Claire,TDBot)
if TDBot.type._ == "userTypeDeleted" then
KickUser(msg.chat_id,TDBot.id)
end
end
...
if matches and (matches:match('^unmute (%d+)$') or matches:match('^حذف سکوت (%d+)$')) and is_JoinChannel(msg) then
local mutes =  matches:match('^unmute (%d+)$') or matches:match('^حذف سکوت (%d+)$')
...
SendMetion(msg.chat_id,mutes, msg.id, '⌯ کاربر :\n( '..mutes..' )\nاز حالت سکوت خارج شد', 12,string.len(mutes))
end
if (matches == 'lock group' or matches == 'قفل گروه') and is_JoinChannel(msg) then
redis:set('MuteAll:'..msg.chat_id,true)
sendText(msg.chat_id, msg.id,'⌯ قفل گروه فعال شد\nلطفا پیامی ارسال نکنید' ,'md')
end
if (matches == 'unlock group' or matches == 'بازکردن گروه') and is_JoinChannel(msg) then
redis:del('MuteAll:'..msg.chat_id)
local mutes =  redis:smembers('Mutes:'..msg.chat_id)
for k,v in pairs(mutes) do
redis:srem('MuteList:'..msg.chat_id,v)
...
-----------Delete All-------------
if (matches == 'deluser' or matches == 'پاکسازی پیام') and is_JoinChannel(msg) then
function DelallByReply(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id, "⌯ من نمیتوانم پیام های خودم را پاک کنم", 'md')
return false
end
if private(msg.chat_id,TDBot.sender_user_id) then
print '                     Private                          '
sendText(msg.chat_id, msg.id, "⌯ من نمیتوانم پیام های یک فرد دارای  مقام را پاک کنم ", 'md')
else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ تمام پیام های :\n ( '..TDBot.sender_user_id..' )\nپاکسازی شد', 12,string.len(TDBot.sender_user_id))
deleteMessagesFromUser(msg.chat_id,TDBot.sender_user_id) 
end
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
if matches and (matches:match('^deluser @(.*)') or matches:match('^پاکسازی پیام @(.*)')) and is_JoinChannel(msg) then
local username = matches:match('^deluser @(.*)') or matches:match('^پاکسازی پیام @(.*)')
function DelallByUsername(Claire,TDBot)
if tonumber(TDBot.id) == tonumber(TD_ID) then
  sendText(msg.chat_id, msg.id, "⌯ من نمیتوانم پیام های خودم را پاک کنم", "md")
return false
    end
  if private(msg.chat_id,TDBot.id) then
print '                     Private                          '
sendText(msg.chat_id, msg.id, "⌯ من نمیتوانم پیام های یک فرد دارای  مقام را پاک کنم ", "md")
else
if TDBot.id then
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ تمام پیام های :\n( '..TDBot.id..' )\nپاکسازی شد', 12,string.len(TDBot.id))
deleteMessagesFromUser(msg.chat_id,TDBot.id) 
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
end
...
if matches and (matches:match('^deluser (%d+)') or matches:match('^پاکسازی پیام (%d+)')) and is_JoinChannel(msg) then
local user_id = matches:match('^deluser (%d+)') or matches:match('^پاکسازی پیام (%d+)')
...
SendMetion(msg.chat_id,user_id, msg.id, '⌯ تمام پیام های :\n( '..user_id..' )\nپاکسازی شد', 12,string.len(user_id))
...
if matches == 'banlist' or matches == 'لیست بن' then
...
if (matches == 'clean bans' or matches == 'پاکسازی لیست بن') and is_JoinChannel(msg) then
local function Clean(Claire,TDBot)
for k,v in pairs(TDBot.members) do
...
sendText(msg.chat_id, msg.id,  '⌯ تمام کابران محروم شده از لیست محرومین حذف شدند ', 'md')
getChannelMembers(msg.chat_id, "Banned", 0, 100000000000,Clean)
end
if matches == 'clean silents' or  matches == 'پاکسازی لیست سکوت' then
local mute = redis:smembers('MuteList:'..msg.chat_id)
for k,v in pairs(mute) do
redis:del('MuteList:'..msg.chat_id)
mute(msg.chat_id, v,'Restricted',   {1, 0, 0, 0, 0,0})
end
sendText(msg.chat_id, msg.id,  '⌯ تمام افراد سکوت شده ازاد شدند', 'md')
end
if (matches == 'clean bots' or matches == 'پاکسازی ربات ها') and is_JoinChannel(msg) then
local function CleanBot(Claire,TDBot)
for k,v in pairs(TDBot.members) do
if tonumber(v.user_id) == tonumber(TD_ID) then
return false
end
...
sendText(msg.chat_id, msg.id,  '⌯ تمام ربات ها اخراج شدند', 'md')
end
if (matches == 'vip' or matches == 'ویژه') and is_JoinChannel(msg) then
function SetVipByReply(Claire,TDBot)
if redis:sismember('Vip:'..msg.chat_id, TDBot.sender_user_id) then
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nاز قبل در لیست افراد ویژه قرار داشت', 12,string.len(TDBot.sender_user_id))
else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nبه لیست افراد ویژه اضافه شده است', 12,string.len(TDBot.sender_user_id))
redis:sadd('Vip:'..msg.chat_id, TDBot.sender_user_id)
end
end
getMessage(msg.chat_id, tonumber(msg.reply_to_message_id),SetVipByReply)
end
if matches and (matches:match('^vip @(.*)') or matches:match('^ویژه @(.*)')) and is_JoinChannel(msg) then
local username = matches:match('^vip @(.*)') or matches:match('^ویژه @(.*)')
function SetVipByUsername(Claire,TDBot)
if TDBot.id then
print('SetVip\nBy : '..msg.sender_user_id..'\nUser : '..TDBot.id..'\nUserName : '..username)
if redis:sismember('Vip:'..msg.chat_id,TDBot.id) then
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nاز قبل در لیست افراد ویژه قرار داشت', 12,string.len(TDBot.id))
else
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nبه لیست افراد ویژه اضافه شده است', 12,string.len(TDBot.id))
redis:sadd('Vip:'..msg.chat_id, TDBot.id)
end
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
...
if (matches == 'clean vip' or matches == 'پاکسازی لیست ویژه') and is_JoinChannel(msg) then
redis:del('Vip:'..msg.chat_id)
sendText(msg.chat_id, msg.id,  '⌯ لیست ویژه پاکسازی شد', 'md')
end
if matches == 'remvip' or matches == 'حذف ویژه' then
function RemVipByReply(Claire,TDBot)
if redis:sismember('Vip:'..msg.chat_id, TDBot.sender_user_id) then
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nاز لیست ویژه خارج شد', 12,string.len(TDBot.id))
redis:srem('Vip:'..msg.chat_id, TDBot.sender_user_id)
else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nاز قبل در لیست ویژه نبود', 10,string.len(TDBot.id))
end
end
...
function RemVipByUsername(Claire,TDBot)
if TDBot.id then
print('RemVip\nBy : '..msg.sender_user_id..'\nUser : '..TDBot.id..'\nUserName : '..username)
if redis:sismember('Vip:'..msg.chat_id,TDBot.id) then
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nاز لیست ویژه خارج شد', 12,string.len(TDBot.id))
...
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nاز قبل در لیست ویژه نبود', 12,string.len(TDBot.id))
end
else 
text = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, text, 'md')
...
if (matches == 'mute' or matches == 'سکوت') and is_JoinChannel(msg) then
function Restricted(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
...
mute(msg.chat_id, TDBot.sender_user_id,'Restricted',   {1, 0, 0, 0, 0,0})
redis:sadd('MuteList:'..msg.chat_id,TDBot.sender_user_id)
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, '⌯ کاربر :\n( '..TDBot.sender_user_id..' )\nدر حالت سکوت قرار گرفت', 12,string.len(TDBot.sender_user_id))
end
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
if matches and (matches:match('^mute @(.*)') or matches:match('^سکوت @(.*)')) and is_JoinChannel(msg) then
local username = matches:match('^mute @(.*)') or matches:match('^سکوت @(.*)')
function MuteuserByUserName(Claire,TDBot)
if tonumber(TDBot.id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
...
mute(msg.chat_id, TDBot.id,'Restricted',   {1, 0, 0, 0, 0,0})
redis:sadd('MuteList:'..msg.chat_id,TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, '⌯ کاربر :\n( '..TDBot.id..' )\nدر حالت سکوت قرار گرفت', 12,string.len(TDBot.id))
end
else 
...
if matches and (matches:match('^mute (%d+)$') or matches:match('^سکوت (%d+)$')) and is_JoinChannel(msg) then
local mutess = matches:match('^mute (%d+)$') or matches:match('^سکوت (%d+)$')
...
SendMetion(msg.chat_id,mutess, msg.id, '⌯ کاربر :\n( '..mutess..' )\nدر حالت سکوت قرار گرفت', 12,string.len(mutess))
end
end
if matches1 and (matches1:match('^([Ss]etflood) (.*)$') or matches1:match('^تنظیم پیام مکرر (.*)$')) and is_JoinChannel(msg) then
local matches1 = matches1:gsub("تنظیم پیام مکرر", "setflood")
...
if matches1 and (matches1:match('^([Ss]etadd) (.*)$') or matches1:match('^تنظیم اداجباری (.*)$')) and is_JoinChannel(msg) then
local matches1 = matches1:gsub("تنظیم اداجباری", "setadd")
local status = {string.match(matches1, "^([Ss]etadd) (.*)$")}
if status[2] == 'newuser' or status[2] == 'کاربران جدید' then
...
if status[2] == 'all' or status[2] == 'همه' then
redis:set("Force:Status:"..msg.chat_id,'all') 
sendText(msg.chat_id, msg.id,'⌯ وضعیت افزودن اجباری برای همه فعال شد', 'md')
end
end
if matches and (matches:match('^mute (%d+)$') or matches:match('^سکوت (%d+)$')) and is_JoinChannel(msg) then
local times = matches:match('^mute (%d+)$')  or matches:match('^سکوت (%d+)$')
time = times * 3600
local function Restricted(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم خودم را محدود کنم ', 'md')
return false
end
...
mute(msg.chat_id, TDBot.sender_user_id,'Restricted',   {1,msg.date+time, 0, 0, 0,0})
redis:sadd('MuteList:'..msg.chat_id,TDBot.sender_user_id)
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, "⌯ کاربر :\n( "..(TDBot.sender_user_id or 000).." )\nدر حالت سکوت قرار گرفت برای [ "..times.." ] ساعت", 12,string.len(TDBot.sender_user_id))
end
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
if (matches == 'unmute' or matches == 'حذف سکوت') and is_JoinChannel(msg) then
function Restricted(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
redis:srem('MuteList:'..msg.chat_id,TDBot.sender_user_id)
mute(msg.chat_id,TDBot.sender_user_id,'Restricted',   {1, 1, 1, 1, 1,1})
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, "⌯ کاربر :\n( "..(TDBot.sender_user_id or 0000000).." )\nاز حالت سکوت خارج شد", 12,string.len(TDBot.sender_user_id))
end
if tonumber(msg.reply_to_message_id) == 0 then
else
getMessage(msg.chat_id, tonumber(msg.reply_to_message_id),Restricted)  
...
if matches and (matches:match('^unmute @(.*)') or matches:match('^حذف سکوت @(.*)')) and is_JoinChannel(msg) then
local username = matches:match('^unmute @(.*)') or matches:match('^حذف سکوت @(.*)')
function MuteuserByUserName(Claire,TDBot)
if tonumber(TDBot.id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
...
if TDBot.id then
redis:srem('MuteList:'..msg.chat_id,TDBot.id)
mute(msg.chat_id, TDBot.id,'Restricted',   {1, 1, 1, 1, 1,1})
SendMetion(msg.chat_id,TDBot.id, msg.id, "⌯ کاربر :\n( "..TDBot.id.." )\nاز حالت سکوت خارج شد", 12,string.len(TDBot.id))
else 
sendText(msg.chat_id, msg.id, '⌯ کاربر یافت نشد',  'md')
end
print('Send')
...
function GeTLink(Claire,TDBot)
local getlink = TDBot.content.text or TDBot.content.caption
...
if (matches == 'ban' or matches == 'بن') and is_JoinChannel(msg) and tonumber(msg.reply_to_message_id) > 0 then
function BanByReply(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,"⌯ من نمیتوانم خودم را محدود کنم",  'md' )
return false
end
  if private(msg.chat_id,TDBot.sender_user_id) then
print '                     Private                          '
  sendText(msg.chat_id, msg.id, "⌯ من نمیتوانم یک کاربر دارای مقام را بن کنم", 'md')
    else
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, "⌯ کاربر :\n( "..TDBot.sender_user_id.." )\nبن شد", 12,string.len(TDBot.sender_user_id))
redis:sadd('BanUser:'..msg.chat_id,TDBot.sender_user_id)
KickUser(msg.chat_id,TDBot.sender_user_id)
end
end
getMessage(msg.chat_id, tonumber(msg.reply_to_message_id),BanByReply)
end
if (matches == 'clean filters' or matches == 'پاکسازی لیست فیلتر') and is_JoinChannel(msg) then
redis:del('Filters:'..msg.chat_id)
...
if (matches == 'clean warns' or matches == 'پاکسازی لیست اخطار') and is_JoinChannel(msg) then
...
if matches and (matches:match('^unban (%d+)') or matches:match('^حذف بن (%d+)')) and is_JoinChannel(msg) then
local user_id = matches:match('^unban (%d+)') or matches:match('^حذف بن (%d+)')
if tonumber(user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
...
SendMetion(msg.chat_id,user_id, msg.id, "⌯ کاربر :\n( "..(user_id or 021).." )\nاز لیست محرومین حذف شد", 12,string.len(user_id))
end
if matches and (matches:match('^ban (%d+)') or matches:match('^بن (%d+)')) and is_JoinChannel(msg) then
local user_id = matches:match('^ban (%d+)') or matches:match('^بن (%d+)')
if tonumber(user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,"⌯ من نمیتوانم خودم را بن کنم",  'md' )
return false
end
if private(msg.chat_id,user_id) then
print '                     Private                          '
...
sendText(msg.chat_id, msg.id, '⌯ کاربر `'..user_id..'` بن شد', 'md')
end
end
if matches and (matches:match('^unban @(.*)') or matches:match('^حذف بن @(.*)')) and is_JoinChannel(msg) then
local username = matches:match('unban @(.*)') or matches:match('^حذف بن @(.*)')
function UnBanByUserName(Claire,TDBot)
if tonumber(TDBot.id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
if TDBot.id then
print('UserID : '..TDBot.id..'\nUserName : @'..username)
redis:srem('BanUser:'..msg.chat_id,TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, "⌯ کاربر :\n( "..(TDBot.id or 021).." )\nاز لیست محرومین حذف شد", 12,string.len(TDBot.id))
else 
sendText(msg.chat_id, msg.id, '⌯ کاربر یافت نشد',  'md')
...
if (matches == 'unban' or matches == 'حذف بن') and is_JoinChannel(msg) and tonumber(msg.reply_to_message_id) > 0 then
function UnBan_by_reply(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
redis:srem('BanUser:'..msg.chat_id,TDBot.sender_user_id)
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, "⌯ کاربر :\n( "..TDBot.sender_user_id.." )\nاز لیست محرومین حذف شد",12,string.len(TDBot.sender_user_id))
RemoveFromBanList(msg.chat_id,TDBot.sender_user_id)
 end
getMessage(msg.chat_id, tonumber(msg.reply_to_message_id),UnBan_by_reply)
end
if matches and (matches:match('^ban @(.*)') or matches:match('^بن @(.*)')) and is_JoinChannel(msg) then
local username = matches:match('^ban @(.*)') or matches:match('^بن @(.*)')
print '                     Private                          '
function BanByUserName(Claire,TDBot)
if tonumber(TDBot.id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,"⌯ من نمیتوانم خودم را بن کنم",  'md' )
return false
end
...
if TDBot.id then
redis:sadd('BanUser:'..msg.chat_id,TDBot.id)
KickUser(msg.chat_id,TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, "⌯ کاربر :\n( "..TDBot.id.." )\nبن شد", 12,string.len(TDBot.id))
else 
t = '⌯ کاربر یافت نشد'
sendText(msg.chat_id, msg.id, t,  'md')
end
...
function kick_by_reply(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
...
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, "⌯ کاربر :\n( "..TDBot.sender_user_id.." )\nاخراج شد",12,string.len(TDBot.sender_user_id))
KickUser(msg.chat_id,TDBot.sender_user_id)
RemoveFromBanList(msg.chat_id,TDBot.sender_user_id)
 end
end
...
function KickByUserName(Claire,TDBot)
if tonumber(TDBot.id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,"⌯ من نمیتوانم خودم را اخراج کنم",  'md' )
return false
end
...
if TDBot.id then
KickUser(msg.chat_id,TDBot.id)
RemoveFromBanList(msg.chat_id,TDBot.id)
SendMetion(msg.chat_id,TDBot.id, msg.id, "⌯ کاربر :\n( "..TDBot.id.." )\nاخراج شد", 12,string.len(TDBot.id))
else 
txtt = '⌯ کاربر یافت نشد'
...
if (matches == 'clean res' or matches == 'پاکسازی لیست محدود') and is_JoinChannel(msg) then
local function pro(arg,data)
...
KickUser(msg.chat_id,user_id)
text= '⌯ کاربر :\n( '..user_id..' )\nاخراج شد'
SendMetion(msg.chat_id,user_id, msg.id, text,12, string.len(user_id))
RemoveFromBanList(msg.chat_id,user_id)
end
end
if matches and (matches:match('^setflood (%d+)') or matches:match('^تنظیم پیام مکرر (%d+)')) and is_JoinChannel(msg) then
local num = matches:match('^setflood (%d+)') or matches:match('^تنظیم پیام مکرر (%d+)')
...
if matches and (matches:match('^setadd (%d+)') or matches:match('^تنظیم اداجباری (%d+)')) and is_JoinChannel(msg) then
local num = matches:match('^setadd (%d+)') or matches:match('^تنظیم اداجباری (%d+)')
if tonumber(num) < 2 then
sendText(msg.chat_id, msg.id, '⌯ عددی بزرگتر از *2* بکار ببرید','md')
else
...
if matches and (matches:match('^setautodelete (%d+)') or matches:match('^تنظیم پاکسازی خودکار (%d+)')) and is_JoinChannel(msg) then
local num = matches:match('^setautodelete (%d+)') or matches:match('^تنظیم پاکسازی خودکار (%d+)')
if tonumber(num) < 50 then
sendText(msg.chat_id, msg.id, '⌯ عددی بزرگتر از *50* بکار ببرید','md')
else
redis:set('force:Time:'..msg.chat_id,num)
sendText(msg.chat_id, msg.id, '⌯ زمان پاکسازی خودکار تنظیم شد به : [ *'..num..'* ]', 'md')
end
end
if matches and (matches:match('^setwarn (%d+)') or matches:match('^تنظیم اخطار (%d+)')) and is_JoinChannel(msg) then
local num = matches:match('^setwarn (%d+)') or matches:match('^تنظیم اخطار (%d+)')
if tonumber(num) < 2 then
sendText(msg.chat_id, msg.id, '⌯ عددی بزرگتر از *2* بکار ببرید','md')
else
...
if matches and (matches:match('^setspam (%d+)') or matches:match('^تنظیم کاراکتر (%d+)')) and is_JoinChannel(msg) then
local num = matches:match('^setspam (%d+)') or matches:match('^تنظیم کاراکتر (%d+)')
if tonumber(num) < 40 then
sendText(msg.chat_id, msg.id, '⌯ عددی بزرگتر از *40* بکار ببرید','md')
else
if tonumber(num) > 4096 then
sendText(msg.chat_id, msg.id, '⌯ عددی کوچکتر از *4096* را بکار ببرید','md')
else
...
if matches and (matches:match('^setfloodtime (%d+)') or matches:match('^تنظیم زمان بررسی (%d+)')) and is_JoinChannel(msg) then
local num = matches:match('^setfloodtime (%d+)') or matches:match('^تنظیم زمان بررسی (%d+)')
if tonumber(num) < 1 then
sendText(msg.chat_id, msg.id, '⌯ زمان بررسی باید بیشتر از *1* باشد','md')
else
redis:set('Flood:Time:'..msg.chat_id,num)
sendText(msg.chat_id, msg.id, '⌯ زمان برسی تنظیم شد به : [ *'..num..'* ]', 'md')
end
end
...
sendText(msg.chat_id, msg.id, '⌯ *عددی بین [*1-1000*] را انتخاب کنید', 'md')
else
...
sendText(msg.chat_id, msg.id, '⌯ تعداد :\n[ '..limit..' ]\nپیام پاکسازی شد', 'md')
end
end
...
if (matches == 'menu pv' or matches == 'فهرست خصوصی') and is_JoinChannel(msg) then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ منو کاربری گروه به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
if matches == 'help' or matches == 'راهنما' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
else
...
if matches == 'help pv' or matches == 'راهنما خصوصی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ بخش راهنما به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
if matches == 'tv' or matches == 'تلویزیون' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
else
...
get(BotHelper, msg.chat_id, 0, 0, "tv",0, GetPanel)
end
if matches == 'tv pv' or matches == 'تلویزیون خصوصی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ بخش تلویزیون به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
if matches == 'pay' or matches == 'خرید' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
else
...
get(BotHelper, msg.chat_id, 0, 0, "pay",0, GetPanel)
end
if matches == 'pay pv' or matches == 'خرید خصوصی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ بخش خرید ربات به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
if matches == 'support' or matches == 'پشتیبانی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
else
...
get(BotHelper, msg.chat_id, 0, 0, "supports",0, GetPanel)
end
if matches == 'support pv' or matches == 'پشتیبانی خصوصی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ بخش پشتیبانی ربات به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
local function Get(Claire, TDBots)
local function GetName(Claire, TDBot)
local chat = msg.chat_id
...
if (matches == 'welcome enable' or matches == 'خوشامد فعال') and is_JoinChannel(msg) then
if redis:get('Welcome:'..msg.chat_id) == 'enable' then
sendText(msg.chat_id, msg.id,'⌯ خوشامد از قبل فعال بود' ,'md')
else
sendText(msg.chat_id, msg.id,'⌯ خوشتمد فعال شد' ,'md')
redis:del('Welcome:'..msg.chat_id,'disable')
redis:set('Welcome:'..msg.chat_id,'enable')
end
end
if (matches == 'forcejoin enable' or matches == 'جوین احباری فعال') and is_JoinChannel(msg) then
if redis:get('ForceJoin'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '⌯  جوین اجباری فعال شد' , 'md')
else
sendText(msg.chat_id, msg.id, '⌯ جوین اجباری فعال بود' , 'md')
redis:set('ForceJoin'..msg.chat_id,true)
end
end
if (matches == 'forcejoin disablr' or matches == 'جوین اجباری غیرفعال') and is_JoinChannel(msg) then
if redis:get('ForceJoin:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '⌯ جوین اجباری غیر فعال بود' , 'md')
redis:del('ForceJoin:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '⌯ جوین اجباری غیر فعال شد' , 'md')
end
end
if (matches == 'lock add' or matches == 'قفل اداجباری') and is_JoinChannel(msg) then
if redis:get('forceAdd:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '⌯ اجبارعضویت فعال شد' , 'md')
else
sendText(msg.chat_id, msg.id, '⌯ اجبارعضویت ازقبل فعال بود' , 'md')
redis:set('forceAdd:'..msg.chat_id,true)
end
end
if (matches == 'unlock add' or matches == 'بازکردن اداجباری') and is_JoinChannel(msg) then
if redis:get('forceAdd:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '⌯ اجبارعضویت غیر فعال شد' , 'md')
redis:del('forceAdd:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '⌯ اجبارعضویت ازقبل فعال نبود' , 'md')
end
end
if (matches == 'welcome disable' or matches == 'خوشامد غیرفعال') and is_JoinChannel(msg) then
...
sendText(msg.chat_id, msg.id, '⌯ قفل فروارد ازقبل فعال بود ' , 'md')
else
...
if matches[2] == 'self' or matches[2] == 'فیلم سلفی' then
...
sendText(msg.chat_id, msg.id, '⌯ قفل مکان ازقبل فعال بود ' , 'md')
else
...
if matches[2] == 'service' or matches[2] == 'سرویس' then
...
sendText(msg.chat_id, msg.id, '⌯ قفل فروارد ازقبل غیرفعال بود ' , 'md')
end
end
if matches[2] == 'farsi' or matches[2] == 'فارسی' then
if redis:get('Lock:Arabic:'..msg.chat_id) then
...
sendText(msg.chat_id, msg.id, '⌯ قفل مکان ازقبل غیرفعال بود ' , 'md')
end
end
if matches[2] == 'voice' or matches[2] == 'ویس' then
if redis:get('Mute:Voice:'..msg.chat_id) then
...
if matches1 and (matches1:match('^[Ss]etwelcome (.*)') or matches1:match('^تنظیم خوشامد (.*)'))  and is_JoinChannel(msg)then
local wel = matches1:match('^[Ss]etwelcome (.*)') or matches1:match('^تنظیم خوشامد (.*)')
redis:set('Text:Welcome:'..msg.chat_id,wel)
sendText(msg.chat_id, msg.id,'⌯ خوش آمد گروه با موفقیت ثبت شد', 'md')
end
...
sendText(msg.chat_id, msg.id, '⌯ کلمه :\n( '..word..' )\nبه لیست فیلتر اضافه شد', 'md')
end

if matches and (matches:match('^unfilter +(.*)') or matches:match('^حذف فیلتر +(.*)')) and is_JoinChannel(msg) then
local word = matches:match('^unfilter +(.*)') or matches:match('^حذف فیلتر +(.*)')
...
function WarnByReply(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
sendText(msg.chat_id, msg.id,  '⌯ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
...
sendText(msg.chat_id, msg.id, "من نمیتوانم به یک فرد دارای مقام اخطار بدهم", 'md')
 ...
 local hashwarn = msg.chat_id..':warn'
local warnhash = redis:hget(msg.chat_id..':warn',TDBot.sender_user_id) or 1
if tonumber(warnhash) == tonumber(warn) then
KickUser(msg.chat_id,TDBot.sender_user_id)
RemoveFromBanList(msg.chat_id,TDBot.sender_user_id)
text= "⌯ کاربر :\n( "..TDBot.sender_user_id.." )\nبه علت دریافت بیش از حد پیام از گروه اخراج شد \n اخطار ها : "..warnhash.."/"..warn..""
redis:hdel(hashwarn,TDBot.sender_user_id, '0')
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, text, 12, string.len(TDBot.sender_user_id))
else
local warnhash = redis:hget(msg.chat_id..':warn',TDBot.sender_user_id) or 1
 redis:hset(hashwarn,TDBot.sender_user_id, tonumber(warnhash) + 1)
text= "⌯ کاربر :\n( "..(TDBot.sender_user_id or '0000Null0000').." )\nشما یک اخطار دریافت کردید \nتعداد اخطار های شما:" ..warnhash .. "/" .. warn .. ""
SendMetion(msg.chat_id,TDBot.sender_user_id, msg.id, text, 12, string.len(TDBot.sender_user_id))
end
end
end
...
function UnWarnByReply(Claire,TDBot)
if tonumber(TDBot.sender_user_id) == tonumber(TD_ID) then
...
local warnhash = redis:hget(msg.chat_id..':warn',TDBot.sender_user_id)
local hashwarn = msg.chat_id..':warn'
redis:hdel(hashwarn,TDBot.sender_user_id,'0')
...
function IdByUserName(Claire,TDBot)
if TDBot.id then
text = '`'..TDBot.id..'`'
sendText(msg.chat_id, msg.id, text, 'md')
end
...
function GetPro1(Claire, TDBot)
 if TDBot.photos[0] then
sendPhoto(msg.chat_id, msg.id, 0, 1, nil, TDBot.photos[0].sizes[2].photo.persistent_id,'⌯ تعداد عکس  : '..TDBot.total_count..'\n سایز عکس : '..TDBot.photos[0].sizes[2].photo.size)
else
...
local function Whois(Claire,TDBot)
 if TDBot.first_name then
local username = ec_name(TDBot.first_name)
SendMetion(msg.chat_id,TDBot.id, msg.id,username,0,utf8.len(username))
else
sendText(msg.chat_id, msg.id,'⌯ کاربر یافت نشد','md')
end
end
...
function Webpage(Claire,TDBot)
if TDBot.photo then
sendPhoto(msg.chat_id, msg.id, 0, 1, nil, TDBot.photo.sizes[0].photo.persistent_id,'')
else
...
function GetPro(Claire, TDBot)
Msgs = redis:get('Total:messages:'..msg.chat_id..':'..(msg.sender_user_id or 00000000))
 if TDBot.photos[0] then
print('persistent_id : '..TDBot.photos[0].sizes[2].photo.persistent_id)  
sendPhoto(msg.chat_id, msg.id, 0, 1, nil, TDBot.photos[0].sizes[2].photo.persistent_id,'شناسه گروه : '..msg.chat_id..'\nشناسه شما : '..msg.sender_user_id..'\nتعداد پیام های شما : '..Msgs..'')
else
...
get(BotHelper, msg.chat_id, 0, 0, "nerkh",0, GetPanel)
end
if matches == '' or matches == '' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ نرخ ربات به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
if matches == 'deleteacc' or matches == 'دلیت اکانت' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
else
...
get(BotHelper, msg.chat_id, 0, 0, "del",0, GetPanel)
end
if matches == 'deleteacc pv' or matches == 'دلیت اکانت خصوصی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ لینک دیلیت اکانت به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
get(BotHelper, msg.chat_id, 0, 0, "card",0, GetPanel)
end
if matches == '' or matches == '' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ شماره کارت پشتیبانی ربات به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
if matches == 'payment' or matches == 'پرداخت' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
else
...
get(BotHelper, msg.chat_id, 0, 0, "p",0, GetPanel)
end
if matches == 'payment pv' or matches == 'پرداخت خصوصی' then
function GetPanel(Claire,TDBot)
if TDBot.results and TDBot.results[0] then
sendInline(msg.sender_user_id,0, 0, 1, TDBot.inline_query_id,TDBot.results[0].id)
sendText(msg.chat_id, msg.id, '⌯ بخش پرداخت ربات به خصوصی شما ارسال شد.\nاگر دریافت نکردید به خصوصی ربات پی ام بدید و دوباره دستور بزنید.', 'html')
else
sendText(msg.chat_id, msg.id, '⌯ اتصال با ربات هلپر بر قرار نیست لطفا ربات را استارت کنید','md')
end
end
...
if (matches == 'ping' or matches == 'پینگ') and is_JoinChannel(msg) then
ping =  ''..botname..' فعال است' 
SendMetion(msg.chat_id,msg.sender_user_id, msg.id, "ربات "..ping.."", 5, string.len(msg.sender_user_id))
end
...
if TDBot.first_name then
TDBotName = ec_name(TDBot.first_name)
else  
TDBotName = 'nil'
end
if result.about then
TDBotAbout = result.about
else  
TDBotAbout = 'nil'
end
if result.common_chat_count  then
TDBotcommon_chat_count  = result.common_chat_count 
else 
TDBotcommon_chat_count  = 'nil'
end
Msgs = redis:get('Total:messages:'..msg.chat_id..':'..msg.sender_user_id)
sendText(msg.chat_id, msg.id,  '⌯ لینک شخصی شما : https://t.me/'..check_markdown(TDBot.username)..'\n⌯ نام کوچک شما  : '..check_markdown(TDBotName)..'\n⌯ شناسه شما : '..msg.sender_user_id..'\n⌯ یوزرنیم شما : @'..check_markdown(TDBot.username)..'\n⌯ بیوگرافی : ['..check_markdown(TDBotAbout)..']\n⌯ تعداد گروه های شما با ربات : '..TDBotcommon_chat_count..'\n⌯ مقام شما : '..rankk..'\n⌯ مقام شما : '..rank..'\n⌯ تعداد پیام های شما : '..Msgs..'','md')
end
GetUserFull(msg.sender_user_id,Get)
end
GetUser(msg.sender_user_id,GetName)
end
if (matches == 'gpinfo' or matches == 'اطلاعات گروه') and is_JoinChannel(msg) then
local function FullInfo(Claire,TDBot)
sendText(msg.chat_id, msg.id,'⌯ سوپر گروه : *'..msg.chat_id..'*\n⌯ ادمین های گروه : *'..TDBot.administrator_count..'*\n⌯ افراد بن شده : *'..TDBot.banned_count..'*\n⌯ اعضا گروه : *'..TDBot.member_count..'*\n⌯ درباره گروه : *'..TDBot.description..'*\n⌯ لینک : *'..TDBot.invite_link..'*\n⌯ افراد محدود شده : *'..TDBot.restricted_count..'*', 'md')
end
...
sendText(msg.chat_id,msg.id,"انلاینم😀✋",'md')
end
if matches == 'ربات' or matches == 'bot' then
local Bot = redis:get('rank'..msg.sender_user_id)
if redis:get('rank'..msg.sender_user_id) then
local rankpro = {'انلاینم😙 '..Bot..'','بفرمایید😜  '..Bot..'','بله😜 '..Bot..''}
sendText(msg.chat_id, msg.id,rankpro[math.random(#rankpro)], 'md')
else
local rank = {'جونم','😜بفرمایید','😜بله😙'}
sendText(msg.chat_id, msg.id,rank[math.random(#rank)], 'md')
end
end
if (matches == 'rank' or matches == 'مقام من') and is_JoinChannel(msg) then
if is_sudo(msg) then
rank =  'سودو' 
elseif is_Owner(msg) then
rank =  'صاحب' 
...
SendMetion(msg.chat_id,msg.sender_user_id, msg.id, "مقام شما : "..rank.."",7, string.len(msg.sender_user_id))
end
